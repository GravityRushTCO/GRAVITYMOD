...
public class Preferences {
    ...
    public static void loadLanguage() {
        if (context != null) {
            LanguageManager.currentLanguage = Preferences.with(context).readString("pref_language", "fr");
        }
    }
    ...
    public static void saveLanguage(String lang) {
        if (context != null) {
            ...
            Preferences.with(context).writeString("pref_language", lang);
        }
    }
    ...
    public static void changeFeatureBool(String featureName, int featureNum, boolean bool) {
        ...
        // V157 — toggle 201 (Dance Party) : on stoppe la musique si désactivé,
        // mais on NE la démarre PAS ici (démarrage uniquement depuis le party mode UI).
        if (featureNum == 201 && !bool) {
            try { stopDanceMusic(); } catch (Throwable t) {
                Log.e("Mod_Menu", "DanceParty stop error: " + t.getMessage());
            }
        }
    ...
    // ─── PARTY MUSIC ───────────────────────────────────────────────────────
    // Joue res/raw/gravity_celeste.mp3 (bundlé dans l'APK).
    private static MediaPlayer sDanceMp;
    public static void startDanceMusic() {
        if (sDanceMp != null) { try { sDanceMp.release(); } catch (Exception ignored) {} sDanceMp = null; }
        if (context == null) return;
        try {
            sDanceMp = MediaPlayer.create(context, R.raw.gravity_celeste);
            if (sDanceMp == null) {
                Log.e("Mod_Menu", "PartyMusic: MediaPlayer.create returned null");
                return;
            }
            sDanceMp.setLooping(true);
            sDanceMp.setVolume(1f, 1f);
            sDanceMp.start();
            Log.i("Mod_Menu", "PartyMusic: playing R.raw.gravity_celeste");
        } catch (Exception e) {
            Log.e("Mod_Menu", "PartyMusic start failed: " + e.getMessage());
            try { if (sDanceMp != null) sDanceMp.release(); } catch (Exception ignored) {}
            sDanceMp = null;
        }
    }
    public static void stopDanceMusic() {
        if (sDanceMp != null) {
            try { sDanceMp.stop(); } catch (Exception ignored) {}
            try { sDanceMp.release(); } catch (Exception ignored) {}
            sDanceMp = null;
            Log.i("Mod_Menu", "DanceParty: stopped");
        }
    }

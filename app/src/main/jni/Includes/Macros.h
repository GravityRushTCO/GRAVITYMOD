...
void DobbyPatchWrapper(const char *libName, const char *relative, std::string data, bool apply) {
    ...
    if(it != pExpress.end()) {
        ...
        auto targetMap = KittyMemory::getAddressMap(info.address);
        if (!targetMap.isValid()) {
            LOGE(OBFUSCATE("Invalid target map for patch: %s"), relative);
            return;
        }

        if (!KittyMemory::memRead(info.address, info.original_bytes.data(), info.patch_bytes.size())) {
            LOGE(OBFUSCATE("Failed to read original bytes for: %s"), relative);
            return;
        }

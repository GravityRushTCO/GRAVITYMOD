...
static const std::string SUPABASE_URL = (const char*)OBFUSCATE("https://prywroemrjeidhvcxgrr.supabase.co");
...
namespace License {
    ...
    const std::string SUPABASE_ANON_KEY = (const char*)OBFUSCATE("sb_publishable_QxuT8DZIxauXJwligR5WIA_aVknjmdD");
    ...
    bool IsNewVersionAvailable() {
        return g_NewVersionAvailable.load();
    }
    ...
    std::string GetRemainingTime() {
        ...
        if (expiresStr.empty() || expiresStr == "Jamais" || expiresStr.find("Illimit") != std::string::npos || expiresStr == "Illimité / Inconnu") {
            return "Illimité";
        }

        int year = 0, month = 0, day = 0, hour = 0, minute = 0, second = 0;
        int parsed = sscanf(expiresStr.c_str(), "%d-%d-%d %d:%d:%d", &year, &month, &day, &hour, &minute, &second);
        if (parsed < 3) {
            parsed = sscanf(expiresStr.c_str(), "%d-%d-%dT%d:%d:%d", &year, &month, &day, &hour, &minute, &second);
        }
        ...
        std::thread([]() {
            ...
            while (true) {
                ...
                if (!g_Unlocked.load() && !s_licenseInFlight.exchange(true)) {
                    std::string url = SUPABASE_URL + (const char*)OBFUSCATE("/rest/v1/rpc/verify_license");
                    ...
                    HttpUtils::PostAsync(url, jsonBody, [](const std::string& response) {
                        ...
                        if (response.empty()) {
                            ...
                            if (s_networkFailCount > 2) {
                                SetStatus(State::OFFLINE, "Connexion perdue avec le serveur de licence");
                        ...
                        if (resJson.is_discarded() || resJson.contains("code")) {
                            // Supabase error usually contains a "code" or "message" at root if not a valid RPC response
                            std::string errMsg = resJson.is_discarded() ? response : resJson.value("message", "Erreur serveur");
                        ...
                        if (status == "valid") {
                            ...
                            SetStatus(State::BANNED, message.empty() ? "VOTRE ABONNEMENT EST EXPIRÉ. Contactez @Gravity_TCO pour renouveler." : message);
                            g_Unlocked.store(false);
                            LOGI("License: EXPIRED");
                        } else if (status == "pending") {
                            SetStatus(State::PENDING, message.empty() ? "APPAREIL EN ATTENTE D'APPROBATION" : message);
                            g_Unlocked.store(false);
                ...
                if (g_Unlocked.load() && !s_cmdInFlight.exchange(true)) {
                    std::string cmdUrl = SUPABASE_URL
                        + (const char*)OBFUSCATE("/rest/v1/player_commands?hwid=eq.") + g_HWID + (const char*)OBFUSCATE("&ack=eq.false");
                ...
                if (g_Unlocked.load()) {
                    ...
                    if (versionCheckTicks >= 10) {
                        versionCheckTicks = 0;
                        std::string configUrl = SUPABASE_URL + (const char*)OBFUSCATE("/rest/v1/app_config?key=eq.version&select=value");
                        HttpUtils::GetAsync(configUrl, [](const std::string& response) {
                            if (!response.empty()) {
                                json configJson = json::parse(response, nullptr, false);
                                if (!configJson.is_discarded() && configJson.is_array() && !configJson.empty()) {
                                    std::string latestVersion = configJson[0].value("value", "");
                                    if (!latestVersion.empty() && latestVersion != GRAVITY_OTA_VERSION) {
